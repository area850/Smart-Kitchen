...
// Load menu items with cooking time estimates and frozen status
$sql = "SELECT id, name, category, price, ing_1, ing_2, image, cooking_time, frozen FROM food_drink ORDER BY category, name";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['order'])) {
    if (!empty($_POST['food_drink']) && is_array($_POST['food_drink'])) {
        ...
        $stmt = $conn->prepare("SELECT name, price, ing_1, ing_2, image, cooking_time FROM food_drink WHERE name IN ($placeholders) AND frozen = 0");
...
<head>
    ...
    <title>z Boss pastry - Gourmet Experience</title>
    ...
    <style>
        ...
        * {
        ...
        .category-nav {
            position: sticky;
            top: 0;
            background: linear-gradient(135deg, var(--primary) 0%, #7a3f15 100%);
            padding: 15px 0;
            z-index: 90;
...
<body>
    ...
    <div class="category-nav">
        <div class="category-nav-container">
            ...
            foreach ($menu as $category => $items): 
                $icon = isset($categoryIcons[$category]) ? $categoryIcons[$category] : "fa-utensils";
            ?>
                <button class="category-nav-btn" data-category="<?= htmlspecialchars(strtolower(str_replace(' ', '-', $category))) ?>">
                    <i class="fas <?= $icon ?>"></i>
    ...
    <form method="post" id="orderForm">
        ...
        <div class="menu-container" id="<?= htmlspecialchars(strtolower(str_replace(' ', '-', $category))) ?>">
            <h2 class="category-title">
                <i class="fas <?= isset($categoryIcons[$category]) ? $categoryIcons[$category] : 'fa-utensils' ?>"></i>
                <?= htmlspecialchars($category) ?>
            </h2>

            <div class="menu-items">
                <?php foreach ($items as $item): ?>
                    <div class="menu-item <?= $item['frozen'] ? 'frozen' : '' ?>">
                        ...
                        <div class="item-details">
                            ...
                            <div class="cooking-time">
                                <i class="fas fa-clock"></i> Cooking time: <?= htmlspecialchars($item['cooking_time']) ?> minutes
                            </div>
                            <div class="item-selection">
                                <div class="item-checkbox">
                                    <input type="checkbox" name="food_drink[]" value="<?= htmlspecialchars($item['name']) ?>" id="item_<?= $item['id'] ?>" <?= $item['frozen'] ? 'disabled' : '' ?>>
                                    <label for="item_<?= $item['id'] ?>" class="<?= $item['frozen'] ? 'disabled-label' : '' ?>">Add to order</label>
                                </div>
    ...
    <footer class="site-footer">
        <div class="footer-content">
            <p>&copy; <?php echo date('Y'); ?> z Boss pastry. All Rights Reserved.</p>
            <p class="copyright">Powerd by <a href="https://neuovacoders.com" target="_blank">Neuova Coders</a></p>
        </div>
    </footer>
    ...
    <script>
        ...
        function updateActiveCategory() {
            ...
            categories.forEach(category => {
                ...
                if (visibleHeight > maxVisible && rect.top <= window.innerHeight / 2) {
        ...
        document.querySelectorAll('.category-nav-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (categoryElement) {
                    ...
                    // Add a highlight effect to the category container
                    categoryElement.style.boxShadow = '0 0 0 3px var(--secondary)';
                    setTimeout(() => {
        ...
        document.querySelectorAll('.place-order-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                const checkedItems = document.querySelectorAll('.item-checkbox input[type="checkbox"]:checked:not([disabled])');
                ...
                const grandTotal = totalPrice + vat + serviceTax;
                
                Swal.fire({
                    title: 'Confirm Your Order',
                    html: `
                        <div style="text-align: left;">
                            <p>You are about to place an order for <b>${totalItems} items</b> with:</p>
                            <ul style="padding-left: 20px; margin-top: 5px;">
                                ${itemsSummary.map(item => `<li>${item}</li>`).join('')}
                            </ul>
                            <div style="margin-top: 15px; border-top: 1px solid #eee; padding-top: 10px;">
                                <p><b>Subtotal:</b> ${totalPrice.toFixed(2)} ETB</p>
                                <p><b>VAT (15%):</b> ${vat.toFixed(2)} ETB</p>
                                <p><b>Service Tax (5%):</b> ${serviceTax.toFixed(2)} ETB</p>
                                <p style="font-weight: bold; font-size: 1.1rem;"><b>Grand Total:</b> ${grandTotal.toFixed(2)} ETB</p>
                            </div>
                            <p style="margin-top: 10px; color: var(--primary);">
                                <i class="fas fa-clock"></i> Estimated preparation time: ${formatCookingTime(totalCookingTime)}
                            </p>
        ...
        document.getElementById('orderSummary').addEventListener('click', () => {
            const checkedItems = Array.from(document.querySelectorAll('.item-checkbox input[type="checkbox"]:checked:not([disabled])'))
                .map(checkbox => {
                    const itemId = checkbox.id.replace('item_', '');
